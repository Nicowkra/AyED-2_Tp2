...
public class SistemaSIU {
    ...
    public SistemaSIU(InfoMateria[] infoMaterias, String[] libretasUniversitarias){
       ...
       this._materias = new ArrayList<Materia>();               //O(1)
...
//Las complejidades de los dos primeros for, de O(M * |n| * (|c| + |m|)), son iguales a O(M * |n| * |c| + |M| * |n| * |m|))
//Separando, por un lado O(M * |n| * |c|) es igual a O(Σ (c ∈ C) |c| * |Mc| ). Esto es así porque lo que se hace es recorrer todas las materias con todos sus nombres, y esto es lo mismo que recorrer todas las carreras y todas las materias de estas.
//Reagrupando, nos queda O(Σ (c ∈ C) |c| * |Mc|) + |M| * |n| * |m|), pero  |M| * |Nm| * |m| representa la longitud del conjunto de nombres de la materia m multiplicado por la cantidad de caracteres de la materia, y eso multiplicado por la cantidad total de materias.
